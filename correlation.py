...
class CorrelationMatrix(object):
    ...
    def far_frr(self, correlation_matrix):
        ...
        print(
            f"Intra-class similarity - Mean: {intra_distances.mean():.4f}, Std: {intra_distances.std():.4f}"
        )
        print(
            f"Inter-class similarity - Mean: {inter_distances.mean():.4f}, Std: {inter_distances.std():.4f}"
        )
    ...
    def adaptive_threshold(self, correlation_matrix):
        ...
        for i in range(n_samples):
            ...
            if i == 0:
                ...
                print(
                    f"Inter-class indices: {np.where(inter_mask)[0][:10]}..."
                )  # Show first 10
                print(
                    f"Intra count: {np.sum(intra_mask)}, Inter count: {np.sum(inter_mask)}"
                )
        ...
        ax2.axhline(
            y=0, color="red", linestyle="--", alpha=0.7, label="No Gap (Overlap)"
        )
        ...
        print(f"\nSeparability Analysis:")
        print(
            f"Samples with positive gap (separable): {positive_gap_samples}/{n_samples} ({positive_gap_samples / n_samples * 100:.1f}%)"
        )
        print(
            f"Samples with negative gap (overlapping): {negative_gap_samples}/{n_samples} ({negative_gap_samples / n_samples * 100:.1f}%)"
        )
